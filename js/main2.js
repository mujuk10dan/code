...
function init(){
	...
	var count=1;
	$('#plastics_bg .particles').css({'margin-top':-_h+'px'}).addClass('add_motion')
	$(window).bind("scroll",function(){
		var sc=Math.floor((($(window).scrollTop()-start_ratio) /(($(document).height()-start_ratio) - $(window).height())) * 100);
		$('#plastics_bg .particles').css({'top':_h*(sc/100)+'px','margin-top':-_h+'px'})
		$('#plastics_bg2 .stacked').css({'top':(100-sc)+'%'});
	...
	$('#plastics_bg2 .stacked .p').each(function(e){
		$(this).css({'top': trace*(Math.random())+(100-trace)+'%','left':Math.random()*100+'vw'})
	})
}
